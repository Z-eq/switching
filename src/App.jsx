...
function SFPPort({ active }) {
  const t = useTheme();
  return (
    <div style={{width:36,height:20,background:active?t.sfpBgActive:t.sfpBgInactive,border:`1px solid ${active?t.sfpBorderActive:t.sfpBorderInactive}`,borderRadius:3,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 4px",boxShadow:active?"0 0 6px #00e67618":"none"}}>
      <div style={{width:6,height:6,borderRadius:"50%",background:active?"#00e676":t.sfpLedInactive,boxShadow:active?"0 0 4px #00e676":"none",animation:active?"pulse-green 3s ease-in-out infinite":"none"}}/>
...
function SwitchChassis({ stackId, ports, selectedPort, onPortClick }) {
  const t = useTheme();
  const cols = Array.from({length:24},(_,c)=>[ports[c*2],ports[c*2+1]]);
  const up=ports.filter(p=>p.status==="up").length, uu=ports.filter(p=>p.isUnused).length;
  return (
    <div style={{background:t.chassisBg,border:`2px solid ${t.chassisBorder}`,borderRadius:8,marginBottom:6,animation:"chassis-glow 6s ease-in-out infinite",overflow:"hidden"}}>
      {/* Top bar */}
      <div style={{background:t.chassisTopBg,borderBottom:`1px solid ${t.chassisTopBorder}`,padding:"6px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",color:t.ciscoColor,fontSize:16,fontWeight:900,letterSpacing:3,textShadow:`0 0 20px ${t.ciscoColor}33`}}>CISCO</div>
          <div style={{display:"flex",flexDirection:"column",gap:1}}>
            <div style={{color:t.modelColor,fontSize:8,fontFamily:"'Share Tech Mono',monospace",letterSpacing:2}}>CATALYST 9300-48P-E</div>
            <div style={{color:t.stackLabelColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace"}}>STACK MEMBER {stackId}</div>
          </div>
        </div>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          {[["SYST","#00e676","pulse-green 2s ease-in-out infinite"],["STAT","#ffd740","pulse-amber 3s ease-in-out infinite"],["DUPLX","#00e676",null],["SPEED","#ffd740",null],["PoE","#ffd740",null]].map(([l,c,a])=>(
            <div key={l} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
              <div style={{width:7,height:7,borderRadius:"50%",background:c,boxShadow:`0 0 6px ${c},0 0 12px ${c}44`,animation:a||"none"}}/>
              <div style={{color:t.ledLabel,fontSize:6,fontFamily:"'Share Tech Mono',monospace"}}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:8,fontFamily:"'Share Tech Mono',monospace"}}>
          <div style={{textAlign:"right"}}>
            <div style={{color:"#00c853",fontSize:11,fontWeight:"bold"}}>{up}/{ports.length}</div>
            <div style={{color:t.stackLabelColor,fontSize:7}}>PORTS UP</div>
          </div>
          {uu>0&&<div style={{textAlign:"right"}}><div style={{color:"#e85530",fontSize:11,fontWeight:"bold"}}>{uu}</div><div style={{color:t.stackLabelColor,fontSize:7}}>UNUSED</div></div>}
        </div>
      </div>
      {/* Port area */}
      <div style={{padding:"10px 14px 8px",display:"flex",gap:10,alignItems:"center"}}>
        <div style={{flex:1,display:"flex",alignItems:"center",gap:0}}>
          {Array.from({length:4},(_,g)=>{
            const gc=Array.from({length:12},(_,c)=>cols[g*12+c]);
            return (
              <div key={g} style={{display:"flex",alignItems:"center",gap:0}}>
                <div style={{background:t.portGroupBg,border:`1px solid ${t.portGroupBorder}`,borderRadius:4,padding:"5px 5px 4px",boxShadow:t.portGroupShadow}}>
                  <div style={{display:"flex",gap:2,marginBottom:3}}>
                    {gc.map((_,c)=><div key={c} style={{width:32,textAlign:"center",color:t.portNumColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace"}}>{g*24+c*2+1}</div>)}
                  </div>
                  <div style={{display:"flex",gap:2,marginBottom:2}}>
                    {gc.map(([top]=[],c)=>top?<RJ45Port key={top.id} port={top} selected={selectedPort?.id===top.id} onClick={()=>onPortClick(top)}/>:<div key={c} style={{width:32}}/>)}
                  </div>
                  <div style={{display:"flex",gap:2,marginBottom:3}}>
                    {gc.map(([,bot]=[],c)=>bot?<RJ45Port key={bot.id} port={bot} selected={selectedPort?.id===bot.id} onClick={()=>onPortClick(bot)}/>:<div key={c} style={{width:32}}/>)}
                  </div>
                  <div style={{display:"flex",gap:2}}>
                    {gc.map((_,c)=><div key={c} style={{width:32,textAlign:"center",color:t.portNumColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace"}}>{g*24+c*2+2}</div>)}
                  </div>
                </div>
                {g<3&&<div style={{width:10,alignSelf:"stretch",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><div style={{width:2,height:"70%",background:t.separatorColor,borderRadius:1}}/></div>}
              </div>
            );
          })}
        </div>
        <div style={{borderLeft:`1px solid ${t.chassisBorder}`,paddingLeft:10,display:"flex",flexDirection:"column",gap:4}}>
          <div style={{color:t.portNumColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace",marginBottom:2,letterSpacing:1}}>SFP+</div>
          {[true,true,false,false].map((a,i)=><SFPPort key={i} active={a}/>)}
        </div>
        <div style={{borderLeft:`1px solid ${t.chassisBorder}`,paddingLeft:10,display:"flex",flexDirection:"column",gap:5}}>
          {[["CON","#78909c"],["USB","#78909c"],["MGT",t.ciscoColor]].map(([l,c])=>(
            <div key={l} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
              <div style={{width:20,height:14,background:t.consoleBg,border:`1px solid ${c}44`,borderRadius:2}}/>
              <div style={{color:t.portNumColor,fontSize:6,fontFamily:"'Share Tech Mono',monospace"}}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{borderLeft:`1px solid ${t.chassisBorder}`,paddingLeft:10,display:"flex",flexDirection:"column",gap:4,alignItems:"center"}}>
          <div style={{color:t.portNumColor,fontSize:6,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1}}>PWR</div>
          <div style={{width:10,height:10,borderRadius:"50%",background:"#00e676",boxShadow:"0 0 8px #00e676,0 0 16px #00e67644",animation:"pulse-green 3s ease-in-out infinite"}}/>
          <div style={{color:t.stackLabelColor,fontSize:6,fontFamily:"'Share Tech Mono',monospace"}}>ON</div>
        </div>
      </div>
      {/* Footer */}
      <div style={{background:t.chassisFooterBg,borderTop:`1px solid ${t.chassisFooterBorder}`,padding:"4px 14px",display:"flex",justifyContent:"space-between"}}>
        <div style={{color:t.footerTextColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace"}}>WS-C9300-48P | 48x GE RJ45 PoE+ | 4x 10G SFP+ | 802.3at | CISCO IOS-XE</div>
        <div style={{display:"flex",gap:12}}>
          {[["UP",ports.filter(p=>p.status==="up").length,"#00c853"],["DOWN",ports.filter(p=>p.status==="down").length,"#f44336"],["UNUSED",uu,"#e85530"],["PoE",ports.filter(p=>p.poe).length,"#f59e0b"]].map(([l,v,c])=>(
            <div key={l} style={{display:"flex",gap:3,alignItems:"center"}}>
              ...
              <span style={{color:t.footerTextColor,fontSize:7,fontFamily:"'Share Tech Mono',monospace"}}>{l}: </span>
